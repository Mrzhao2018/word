...
pub fn dwarf_movement_system(
    ...
    for (mut transform, mut grid_pos, velocity) in query.iter_mut() {
        ...
        if velocity.x.abs() > 0.01 || velocity.y.abs() > 0.01 {
            // 计算移动方向（规范化到-1, 0, 1）
            let dir_x = if velocity.x.abs() < 0.01 { 0 } else { velocity.x.signum() as i32 };
            let dir_y = if velocity.y.abs() < 0.01 { 0 } else { velocity.y.signum() as i32 };
            ...
            if target_grid_x >= 0 && target_grid_x < WORLD_WIDTH 
                && target_grid_y >= 0 && target_grid_y < WORLD_HEIGHT {
            ...
            if can_move {
                // 计算目标世界坐标（与地形对齐）
                let target_x = target_grid_x as f32 * TILE_SIZE - (WORLD_WIDTH as f32 * TILE_SIZE / 2.0) + (TILE_SIZE / 2.0);
                let target_y = target_grid_y as f32 * TILE_SIZE - (WORLD_HEIGHT as f32 * TILE_SIZE / 2.0) + (TILE_SIZE / 2.0);
        ...
        let calculated_grid_x = ((transform.translation.x + (WORLD_WIDTH as f32 * TILE_SIZE / 2.0) - (TILE_SIZE / 2.0)) / TILE_SIZE).round() as i32;
        let calculated_grid_y = ((transform.translation.y + (WORLD_HEIGHT as f32 * TILE_SIZE / 2.0) - (TILE_SIZE / 2.0)) / TILE_SIZE).round() as i32;
        
        // 只在合理范围内更新GridPosition
        if calculated_grid_x >= 0 && calculated_grid_x < WORLD_WIDTH 
            && calculated_grid_y >= 0 && calculated_grid_y < WORLD_HEIGHT {
